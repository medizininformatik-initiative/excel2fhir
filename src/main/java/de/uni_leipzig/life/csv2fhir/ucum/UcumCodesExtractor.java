...
/**
 * This class can be used to extract all valid UCUM codes and ist display
 * string from the JSON file of the hl7 core. This json file is used to
 * validate the resulting json object using UCUM codes.<br>
 * The file json file can be found in the tgz file of the hl7 core, e.g. in
 * core Version 4.0.1 in
...
public class UcumCodesExtractor {
    ...
    public static void main(String[] args) throws IOException {
        ...
        BothDirectionResourceMapper mapper = new BothDirectionResourceMapper(
                UCUM_CODE_TO_DISPLAY_MAP_RESOURCE_FILE_NAME);
    ...
    /**  */
    private static final File RESOURCE_SUB_DIR = new File(ApplicationManager.getApplicationDir(),
            "src/main/resources/");

    /**
     * source JSON file fom which the codes and displays must to be extracted
     */
    private static final File UCUM_CONCEPTS_JSON_SOURCE_FILE = new File(RESOURCE_SUB_DIR,
            "ucum/ValueSet-ucum-common.json");
    ...
    /**
     * Extracts the map UCUM_Codes.map from the json-file with all valid UCUM codes
    ...
    private static void extractUCUMCodesAndDisplay() {
        ...
        if (FileHandler.guaranteeWriteableFile(targetMapFile)) {
            JSONFilesConverter jsonFilesConverter = new JSONFilesConverter(UCUM_CONCEPTS_JSON_SOURCE_FILE,
                    targetMapFile) {
                ...
                public void convert(JSONObject objectToConvert) {
                    ...
                    for (Object jsonObject : ucumConceptsArray) {
                        ...
                        String code = getString(ucumConcept, "code").replaceAll(" ", "\\\\u0020"); // whitespaces in the
                                                                                                   // key codes must be
                                                                                                   // UTF-8 encoded in
                                                                                                   // the map file
    ...
    private static void extractSynonymsMap(BothDirectionResourceMapper ucumCodeToDisplayMapper) throws IOException {
        File targetMapFile = new File(RESOURCE_SUB_DIR,
                UCUM_AUTOMATIC_CREATED_SYNONYM_TO_UCUM_CODE_MAP_RESOURCE_FILE_NAME);
        if (FileHandler.guaranteeWriteableFile(targetMapFile)) {
            ...
            try (BufferedReader in = new BufferedReader(new InputStreamReader(ucumConceptsTSVFile.openStream()))) {
                ...
                for (CSVRecord record : csvParser) {
                    ...
                    code = code.replace('´', '\'').replace('`', '\''); // all 'new' valid UCUM codes only use ' instead
                                                                       // of ` or ´
                    if (validUcumCodes.contains(code)) {
                        ...
                        for (String synonym : synonyms) {
                            ...
                            if (!validUcumCodes.contains(synonym)) {
                                synonym = synonym.replaceAll(" ", "\\\\u0020"); // whitespaces in the key codes must be
                                                                                // UTF-8 encoded in the map file
     ...
     * @return
     */
    private static void addDefaultSynonyms(BothDirectionResourceMapper ucumCodeToDisplayMapper,
            BothDirectionResourceMapper synonymToValidUcumCodeMapper) {
        List<String> synonyms = new ArrayList<>(synonymToValidUcumCodeMapper.keySet()); // we need a copy because we
                                                                                        // change the map in the loop
        for (String synonym : synonyms) {
     ...
     * @param code2Convert
     */
    private static final void addSynonym(BothDirectionResourceMapper ucumCodeToDisplayMapper,
            BothDirectionResourceMapper synonymToValidUcumCodeMapper, String validUcumCode, String code2Convert) {
        ...
        if (lowerCaseDisplay.contains("liter") || lowerCaseDisplay.contains("liiter")) { // typo in json file for code
                                                                                         // ng/mL -> nanogram per
                                                                                         // millliiter
