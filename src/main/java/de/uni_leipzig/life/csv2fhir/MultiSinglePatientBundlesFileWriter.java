...
public class MultiSinglePatientBundlesFileWriter {
     ...
     * @throws Exception
     */
    private MultiSinglePatientBundlesFileWriter(File outputDirectory, String outputFileNameBase,
            FHIRValidator validator, boolean writeNDJsonFile, boolean writeZipFile) throws Exception {
     ...
     * @return a writer for json.zip and ndjson files, if the given outputFileTypes
     *         contains the keys of this files types. If no key found in the
     *         outputFileTypes then <code>null</code> is returned.
     * @throws Exception
     */
    public static MultiSinglePatientBundlesFileWriter create(File outputDirectory, String outputFileNameBase,
            FHIRValidator validator, OutputFileType... outputFileTypes) throws Exception {
        ...
        return new MultiSinglePatientBundlesFileWriter(outputDirectory, outputFileNameBase, validator, writeNDJsonFile,
                writeZipFile);
    }
                    ...
                    if (zipJsonOutputStream != null) {
                        ...
                        try (InputStream bundleInputStream = new ByteArrayInputStream(encodedBundle.getBytes(UTF_8))) {
                            ...
                            ZipEntry zipEntry = new ZipEntry(
                                    outputFileNameBase + pid + ZIPJSON.getBaseFileType().getFileExtension());
    ...
    /**
     * Searches in all bundle entries for a resource of class {@link Patient} and
     * returns its ID. If there are more than one patients in the bundle than the
     * very first ID will be returned.
     *
     * @param bundle
     * @return
     */
